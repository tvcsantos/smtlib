...
abstract class AbstractSMTSolver(fsimpl:Simplifier[Formula],
    ...
    fsubst:SubstitutionVisitor[Formula]) extends SMTSolver {
  ...
  protected def checkSatImplWithModel(as:Set[(Formula, AST)], 
      tm:Map[Type, TypeDef], 
      dm:Map[Type, (Set[FunDef], Set[TheoDef])]):
      Option[(SMTResult, Option[SMTModel])] =  {
    import SMTExecutionMode._
    currentExecMode match {
  ...
  protected final def getDefinitionsMaps(a:Formula, tm:Map[Type, TypeDef], 
    ...
    val ndm = getNewDefinitions(hie, tm, dm)
    (ntm, ndm)
  }
  
  protected def getNewDefinitions(types:Set[Type], tm:Map[Type, TypeDef], 
      dm:Map[Type, (Set[FunDef], Set[TheoDef])]):
    	  	Map[Type, (Set[FunDef], Set[TheoDef])]
  ...
  protected final def findSignature(name:String, 
      ...
      dm:Map[Type, (Set[FunDef], Set[TheoDef])]):Option[FunDef] = {
    val s = getFunDef(tm.values)
    var r = s.find(e => hasSignature(e, name, args, ret))
    r match {
      case None =>
        var lr:Option[FunDef] = None
        val y = dm.values.map(x => x._1).iterator
  ...
  protected def getFunDef(s:Iterable[TypeDef]):Seq[FunDef]
  
  protected def mkFunction(f:FunDef, args:Seq[AST]):AST
      ...
      case x:Function =>
        ...
        if (isBuiltInFunction(x))
          ...
          funOpt match {
            ...
            case Some(y) => mkFunction(y, argsASTs)
          }
        }
    }
  }
  ...
  protected final def mkValue(x:Term, tm:Map[Type, TypeDef],
    ...
    def mkValueAux(x:Term, t:Type, tm:Map[Type, TypeDef], 
        dm:Map[Type, (Set[FunDef], Set[TheoDef])]):AST = {
      x match {
        case y:NumLit =>
  ...
  protected def mkPredicate(f:FunDef, args:Seq[AST]):AST
      ...
      case x:Predicate =>
        ...
        if (isBuiltInPredicate(x))
          ...
          funOpt match {
            ...
            case Some(y) => mkPredicate(y, argsASTs)
          }
        }
      case Equality(l, r) =>
  ...
  def getFormulaTypeChecker():TypeChecker[Formula] = ftc
  
  def getFormulaFreeVariablesVisitor():FreeVariablesVisitor[Formula] = ffv
  
  def getFormulaSubstitution():SubstitutionVisitor[Formula] = fsubst
