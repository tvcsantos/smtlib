...
sealed trait Formula extends Typable[Formula] with
	Visitable[Formula] {
  ...
  def typeCheck(c:TypeChecker[Formula], e:Environment[Type]):Formula =
    visit(c, e)
}
...
case class Predicate(name:String, args:List[Term]) 
	extends AbstractFormula(Some(BoolType)) {
