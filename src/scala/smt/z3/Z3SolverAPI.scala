...
class Z3SolverAPI(fsimpl:Simplifier[Formula],
    ...
    extends AbstractSMTSolver(fsimpl, ffv, ftc, fsubst) with SMTSolver {
  
  protected[smt] def newInstance():SMTSolver =
    new Z3SolverAPI(fsimpl, ffv, ftc, fsubst)
  ...
  protected def mkFunction(f:FunDef, args:Seq[AST]):AST = f(args:_*)
