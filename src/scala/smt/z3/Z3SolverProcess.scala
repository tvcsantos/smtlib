...
class Z3SolverProcess(fsimpl:Simplifier[Formula],
    ...
    extends AbstractSMTSolverProcess(fsimpl, ffv, ftc, fsubst) 
	with SMTSolver {
  
  protected[smt] def newInstance():SMTSolver =
    new Z3SolverProcess(fsimpl, ffv, ftc, fsubst)
  ...
  protected def getFunDef(s:Iterable[TypeDef]):Seq[FunDef] = {
    ...
    var r:MutableSet[FunDef] = MutableHashSet[FunDef]()
    for (e <- s) {
      e._2 match {
        case None => ()
        case Some(x) =>
  ...
  protected def processOutput(out: Stream[String]):Option[SMTResult] = 
    out match {
  ...
  protected def runSolver(code:String):Stream[String] = {
    ...
    //println(code)
    //println("----")
    val out = new java.io.FileWriter("test.smt2")
    out.write(code)
	out.close
	import scala.sys.process._
	optTimeout match {
      case None => Process("z3 test.smt2").lines
      case Some(x) => Process(s"z3 -T:${x/1000} test.smt2").lines
    }
  }
